...
namespace AdoNetCore.AseClient.Tests.Integration
{
    ...
    public class SimpleQueryTests
    {
        ...
        public void SelectLongString_Literal_ShouldWork(int count)
        {
            ...
            using (var connection = new AseConnection(ConnectionStrings.BigTextSize))
            {
                Assert.AreEqual(expected, connection.ExecuteScalar<string>($"select '{expected}'"));
            }
        }
...
select (@a * @b)/100").First();
        ...
        public static IEnumerable<TestCaseData> SelectAseDecimal_Parameter_ShouldWork_Cases()
        {
            ...
            yield return new TestCaseData(AseDecimal.Parse("-99999999999999999999999999999999999999")); //-10^38 + 1
            yield return new TestCaseData(AseDecimal.Parse("-100000000000000000000000000000000000000")); //-10^39: should throw IndexOutOfRangeException when trying to bind the parameter
        }


        [TestCaseSource(nameof(SelectAseDecimal_Literal_ShouldWork_Cases))]
        public void SelectAseDecimal_Literal_ShouldWork(string input)
        {
            var expected = AseDecimal.Parse(input);
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                {
                    Assert.AreEqual(expected, (AseDecimal)Convert.ToDecimal(result));
                }
                else
                {
                    Assert.AreEqual(expected, result);
